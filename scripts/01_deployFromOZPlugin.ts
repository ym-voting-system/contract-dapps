...
async function main() {
	...
	let factory = new ethers.ContractFactory(CommownSW.abi, CommownSW.bytecode, owner);
	...
	let proxy = await upgrades.deployProxy(factory, [addresses,confirmation], { kind: 'uups' });

  	//waiting of proxy deployment
  	await proxy.deployed();

  	console.log("CommownSW's Proxy deployed to: "+proxy.address);
	console.log("CommownSW's Implementation (main contract) deployed to: ",await upgrades.erc1967.getImplementationAddress(proxy.address));
}
