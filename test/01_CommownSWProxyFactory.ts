...
import CommownSWProxyFactory from "../src/artifacts/contracts/CommownSWProxyFactory.sol/CommownSWProxyFactory.json";
import CommownSW from "../src/artifacts/contracts/CommownSW.sol/CommownSW.json";
...
describe('01_CommownSWProxyFactory__01_deployProxyFactory', function () {
	it("01_01_01: it deploys the CommownSWProxyFactory and the CommownSW", async function(){
		const CommownSWProxyFactory = await ethers.getContractFactory("CommownSWProxyFactory");
		const proxyFactory = await CommownSWProxyFactory.deploy();
...
describe('01_CommownSWProxyFactory__02_createProxy', function () {
	let CommownSWProxyFactory;
	beforeEach(async function () {
		const CommownSWProxyFactory = await ethers.getContractFactory("CommownSWProxyFactory");
		let proxyFactory = await CommownSWProxyFactory.deploy();

		await proxyFactory.deployed();

		const [sign0, sign1, sign2] = await ethers.getSigners();
	});
	it("01_02_01: it deploys a proxy from sign0", async function(){
		//const proxyCreated = await proxyFactory.createProxy(addresses,confirmation);
	});
	it("01_02_02: it deploys a proxy from sign1", async function(){
		//const proxyCreated = await proxyFactory.createProxy(addresses,confirmation);
	});
});
